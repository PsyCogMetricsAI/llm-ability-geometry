...
R=Path(__file__).resolve().parents[1]; C=R/'config/CT_MEDICAL_REFINEMENT.json';OUT=R/'runs/new_medical_repair_refined'
...
def grid(q):
 ...
 return t,lw
class Objective:
 def __init__(self,y,kind,q=321):self.y=y;self.kind=kind;self.t,self.lw=grid(q);self.I=len(y);self.calls=0
 def unpack(self,z):
...
def fit(y,kind,start,label):
 ...
 val=obj.calc(res.x,True);fine=Objective(y,kind,641).calc(res.x,True);pg=float(np.max(abs(projected(val['gradient'],res.x,bounds))))
 ...
 all_finite=all(np.isfinite(val[k]).all() for k in ['a','b','theta','var','logML','posterior']) and bool(np.all(val['var']>=-1e-10))
 numerical=bool(all_finite and res.success and pg<=.01 and np.isfinite(val['theta']).all() and dt<=.02 and dl<=.02 and not b_hit.any() and (kind=='rasch' or hit.mean()<=.1))
 np.savez_compressed(OUT/(label+'.npz'),Y=y,parameters=res.x,discrimination=val['a'],difficulty=val['b'],theta=val['theta'],variance=val['var'],posterior=val['posterior'],logML=val['logML'],theta641=fine['theta'],logML641=fine['logML'],gradient=val['gradient'],trace=np.array(trace),a_boundary=hit,b_boundary=b_hit)
...
def main():
 c=json.loads(C.read_text());approval=json.loads((R/'verifier/CT_MEDICAL_REFINEMENT_APPROVED.json').read_text());assert approval['contract_sha256']==sha(C)
 ...
 for name,info in c['inputs'].items():
  ...
  records=[]
  for kind,start in [('2pl',1.),('2pl',.7),('map',1.)]:records.append(fit(y,kind,start,name+'_'+kind+'_'+str(start)))
  ...
  for rec in records:rec['score_acceptance_candidate']=bool(rec['numeric_screen_pass'] and (rec['kind']!='2pl' or (stability['passes'] and all(r['numeric_screen_pass'] for r in records if r['kind']=='2pl'))))
 ...
 (R/'reports/NEW_MEDICAL_REPAIR_REFINED.json').write_text(json.dumps(summary,indent=2)+'\n')
